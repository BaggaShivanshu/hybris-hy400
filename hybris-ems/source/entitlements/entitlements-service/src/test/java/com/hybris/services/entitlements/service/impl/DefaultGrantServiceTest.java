...
public class DefaultGrantServiceTest
{
	...
	@Test
	@Transactional
	public void shouldRemoveUserGrants()
	{
		grantBuilder.userId("user1").grant("1", "2", "3");
		grantBuilder.userId("user2").grant("4", "5", "6");

		grantService.deleteUserData("user1");

		assertTrue(grantService.getGrants("user1", null, null, null, null).isEmpty());
		assertEquals(3, grantService.getGrants("user2", null, null, null, null).size());
	}
