...
public class DefaultGrantService implements GrantService
{
	...
	public void deleteUserData(final String userId)
	{
		...
		List<TypedRestriction<Grant>> restrictions = Collections.singletonList(eq(Grant.USERID, userId));
		List<Grant> grants = get(restrictions);
		grants.forEach(persistenceManager::remove);
	}
